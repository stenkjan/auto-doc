...
function ConnectedServicesSection() {
  ...
  const [driveStatus, setDriveStatus] = useState<ServiceStatus | null>(null);
  const [githubStatus, setGithubStatus] = useState<ServiceStatus | null>(null);
  const [loadingDrive, setLoadingDrive] = useState(false);
  const [loadingGithub, setLoadingGithub] = useState(false);
  const [disconnectingDrive, setDisconnectingDrive] = useState(false);

  const isGoogleUser = session?.user?.email?.endsWith("@gmail.com") ||
    (session as { user?: { provider?: string } } | null)?.user?.provider === "google";

  useEffect(() => {
    async function fetchStatuses() {
      try {
        const [driveRes, githubRes] = await Promise.all([
          fetch("/api/services/google-drive"),
          fetch("/api/services/github"),
        ]);
        if (driveRes.ok) setDriveStatus(await driveRes.json());
        if (githubRes.ok) setGithubStatus(await githubRes.json());
      } catch {
        // silently ignore
      }
    }
    if (session?.user) fetchStatuses();
  }, [session]);

  const connectGoogleDrive = () => {
    setLoadingDrive(true);
    window.location.href = "/api/services/connect/google-drive";
  };

  const disconnectGoogleDrive = async () => {
    setDisconnectingDrive(true);
    try {
      await fetch("/api/services/google-drive", { method: "DELETE" });
  ...
  const connectGitHub = () => {
    ...
    window.location.href = "/api/services/connect/github";
  };
  ...
  return (
    <div className="mb-3">
      ...
      <div className="space-y-2">
        {/* Google Drive */}
        <div className="border rounded-xl p-3 bg-white border-gray-200">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-yellow-400 to-green-400 flex items-center justify-center shrink-0">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-gray-400">
                {driveStatus?.connected ? "Verbunden" : "Nicht verbunden"}
              </p>
            </div>
            {driveStatus?.connected ? (
              ...
              <button
                ...
                className="text-xs px-2.5 py-1 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50 transition-colors font-medium"
              >
                {loadingDrive ? "..." : "Verbinden"}
              </button>
            )}
          </div>
        </div>
        ...
        <div className="border rounded-xl p-3 bg-white border-gray-200">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-gray-400">
                {githubStatus?.connected ? "Verbunden" : "Bald verfügbar"}
              </p>
            </div>
            {githubStatus?.connected ? (
              ...
              <button
                ...
                disabled={loadingGithub || !process.env.NEXT_PUBLIC_GITHUB_AVAILABLE}
                title="GitHub-Integration demnächst verfügbar"
                className="text-xs px-2.5 py-1 bg-gray-200 text-gray-500 rounded-lg cursor-not-allowed"
              >
                {loadingGithub ? "..." : "Bald"}
              </button>
            )}
